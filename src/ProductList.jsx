...
function ProductList({ onHomeClick }) {
  ...
  const [showPlants, setShowPlants] = useState(true); // varsayılan olarak bitkileri göster
  const [selectedCategory, setSelectedCategory] = useState(null);
  const [addedToCart, setAddedToCart] = useState({}); // Sepete eklenen ürünleri takip et

  const dispatch = useDispatch();

  // Redux deposundan sepet öğelerini alın
  ...
  // Component yüklendiğinde sepet durumunu güncelle
  useEffect(() => {
    // Sepetteki ürünlerin durumunu takip et
    const newAddedToCart = {};
    cartItems.forEach((item) => {
      newAddedToCart[item.id] = true;
    });
  ...
  // Bitkiler sayfasına geçiş fonksiyonu
  ...
  // Alışverişe devam et fonksiyonu
  const handleContinueShopping = () => {
    setShowCart(false);
    setShowPlants(true);
  };

  // Sepete ekleme fonksiyonu - handleAddToCart
  const handleAddToCart = (plant) => {
    // Redux store'a addItem eylemini gönder
    dispatch(addItem(plant));

    // Ürünün sepete eklendiğini işaretle
    setAddedToCart((prev) => ({
      ...prev,
      [plant.id]: true,
    }));

    // Konsola bir log yazdırarak doğru çalıştığını kontrol edelim
    console.log(`Added ${plant.name} to cart`);
    console.log(`Cart now has ${cartItemCount + 1} items`);
  };
  ...
  return (
    <div>
      ...
      {/* İçerik kısmı: Bitkiler veya Sepet */}
      {showPlants && !showCart ? (
        <div className="product-grid">
          {/* Kategori başlığı */}
          <div className="plantname_heading">
            <h1 className="plant_heading">Plant Categories</h1>
          </div>
                    ...
                    <div className="product-card" key={plant.id}>
                      ...
                      <button
                        className={`product-button ${
                          addedToCart[plant.id] ? "added-to-cart" : ""
                        }`}
